...
public class BinaryTree {
	...
	public static void getAllPaths(TNode node) {
		...
		getPaths(node, array, 0);
	}

	public static void getPaths(TNode node, int array[], int length) {
		if (node == null)
			return;
		...
		if (node.leftChild == null && node.rightChild == null) {
			...
			array[length]=node.data;
			getPaths(node.leftChild, array, length + 1);
			getPaths(node.rightChild, array, length + 1);
		}
